...
class StockSummaryReportXlsx(models.AbstractModel):
    _name = 'report.dr_salim_custom_reports.stock_summary_report_xls'
    ...
    def generate_xlsx_report(self, workbook, data, products):
        ...
        title = workbook.add_format(
            {'bold': True, 'align': 'center', 'valign': 'vcenter', 'font_name': 'Georgia', 'font_size': 18})
        font_format = workbook.add_format(
            {'font_name': 'Georgia', 'font_size': 11, 'bold': True, 'align': 'center', 'bg_color': '#fcba03'})
        ...
        row += 1
        sheet.merge_range(row, col, row + 1, col + 4, 'Stock Summary Report', title)  # Merge range for the title
        row += 2
        ...
        for rec in data['stock']:
            ...
            sheet.write(row, col + 2, rec['category'])
